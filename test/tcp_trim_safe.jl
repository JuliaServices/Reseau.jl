...
function run_tcp_trim_sample()::Nothing
    ...
    try
        ...
        NC.pending_input(server) === :none || error("expected no pending input on an idle connection")
        ...
        NC.pending_input(server) === :none || error("expected no pending input after draining")
        close(client)
        eof(server) || error("expected EOF after peer close")
        NC.pending_input(server) === :eof || error("expected pending_input to report EOF")
    finally
        _close_quiet!(server)
        _close_quiet!(client)
