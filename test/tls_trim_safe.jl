...
const _TLS_SERVER_LISTENER = Ref{Union{Nothing, NC.Listener}}(nothing)
...
function _tls_server_task_entry()::Nothing
    listener = _TLS_SERVER_LISTENER[]::NC.Listener
    conns = TL.Conn[]
    for i in 1:2
        conn = TL.server(NC.accept(listener), _TLS_SERVER_CONFIG)
        TL.handshake!(conn)
        push!(conns, conn)
