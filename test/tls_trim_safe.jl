...
function _run_tls_roundtrip_states!(
    ...
    try
        ...
        TL.pending_input(client) === :none || error("expected no pending TLS input before the ack")
        write(client, UInt8[0x21]) == 1 || error("expected TLS client ack write")
        eof(client) || error("expected TLS EOF")
        TL.pending_input(client) === :eof || error("expected pending_input to report TLS EOF")
