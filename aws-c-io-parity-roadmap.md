...
- [x] Implement full aws-c-io default resolver behavior (cache, TTL rules, refresh frequency)
...
- [x] World-age safe callback invocation from resolver threads and purge paths

### 6.2 Custom resolver vtable
- [x] Support custom `resolve_host` implementation callbacks
- [x] Blocking resolution handling parity
...
- [x] Implement PKCS#11 private key lookup + sign/decrypt helpers
...
- [~] `socket_test.c` -> `test/socket_tests.jl` (extend; port/IP parse + local/UDP comm + wrong-thread + zero-port + bind/outgoing error cases + async write callback + timeout/cancellation + cleanup (connect/accept/write) done, remaining vsock loopback + local socket pipe race + interface bind tests pending)
