...
Core.eval(TCP, quote
    function compiler_repro_wait_arg_kwargs_deadtail(
        ...
        try
            ...
            errno == Int32(0) || errno == Int32(Base.Libc.EISCONN) || return fd
            return fd
        catch
            close!(fd)
            rethrow()
...
Base.return_types(Core.kwcall, Tuple{KW, typeof(TCP.compiler_repro_wait_arg_kwargs_deadtail), TCP.SocketAddrV4})
