...
The remaining sections are still useful as a backlog, but they now
overstate the current gaps in those specific areas.
...
`Reseau.HTTP`'s current server story is object-oriented:
...
This is fine architecturally, but for HTTP.jl parity we likely want thin
function-style wrappers over the current object model.
...
`Reseau.HTTP` currently has no SSE surface.

This is a clean, self-contained parity target that likely matters more than
`download`.
...
`Reseau.HTTP` currently has no WebSocket public API.

This is a large subsystem and should likely be treated as its own parity track.
...
### Phase 3: Function-Style Server Wrappers

Add wrappers over the current server object model:
...
This narrows one of the most visible top-level shape differences with
`HTTP.jl`.
...
### Phase 5: Cookies and Multipart/Form APIs
...
This is a bounded parity target and probably a better near-term investment than
`download`.
...
1. client request surface
2. streaming APIs
3. function-style server entrypoints
4. router/middleware
