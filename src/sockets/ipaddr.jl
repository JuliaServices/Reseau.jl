# This file is adapted from Julia's stdlib `Sockets/src/IPAddr.jl` (MIT licensed).

export IPAddr, IPv4, IPv6, InetAddr, @ip_str
...
struct IPv4 <: IPAddr
    ...
    IPv4(a::UInt8, b::UInt8, c::UInt8, d::UInt8) = new(UInt32(a) << 24 | UInt32(b) << 16 | UInt32(c) << 8 | d)
...
Base.show(io::IO, ip::IPv4) = print(io, "ip\"", ip, "\"")
Base.print(io::IO, ip::IPv4) = print(
    io,
    string((ip.host & (0xFF000000)) >> 24), ".",
...
struct IPv6 <: IPAddr
    ...
    function IPv6(
        ...
        IPv6(UInt16(a), UInt16(b), UInt16(c), UInt16(d), UInt16(e), UInt16(f), UInt16(g), UInt16(h))
    end
end
...
Base.show(io::IO, ip::IPv6) = print(io, "ip\"", ip, "\"")

# RFC 5952 compliant show function
function Base.print(io::IO, ip::IPv6)
    i = 8
    m = 0
    longest_sub_i = -1
    while i != 0
...
function Base.parse(::Type{IPv4}, str::AbstractString)
    fields = split(str, '.')
    i = 1
    ret = 0
    for f in fields
...
function Base.parse(::Type{IPv6}, str::AbstractString)
    fields = split(str, ':')
    if length(fields) > 8
        throw(ArgumentError("too many fields in IPv6 address"))
    ...
    elseif in('.', fields[end])
        return IPv6((parseipv6fields(fields[1:(end - 1)], 6)) | parse(IPv4, fields[end]).host)
    else
        return IPv6(parseipv6fields(fields))
    end
end

function Base.parse(::Type{IPAddr}, str::AbstractString)
    if ':' in str
...
function Base.show(io::IO, addr::InetAddr)
    show(io, typeof(addr))
    print(io, "(")
    show(io, addr.host)
